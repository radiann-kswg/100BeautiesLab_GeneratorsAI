...
- 生成画像は3階層構造 `output/{YYYYMMDD}/{YYYYMMDD_HH}/{ts}_{provider}_{form}_num{NNN}[_suffix]/` へ保存する。
  - `provider`: `gemini` / `openai`、`form`: `corefolder` / `humanoid`、`NNN`: 3 桁ゼロパディングのキャラクター番号
  - `_suffix`: i2i 時の `_iter1` / `_iter2` ... や `_prompt-assist` など
  - 例: `output/20260609/20260609_14/20260609_140532_gemini_corefolder_num057/num057_corefolder_01.png`
...
- 物理レイアウトは `output/{YYYYMMDD}/{YYYYMMDD_HH}/{ts}_{provider}_{form}_num{NNN}/` の3階層 (`作業日 / バッチHH / 実行`)。
