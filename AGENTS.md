...
このファイルは、GitHub Copilot / Claude / GPT Codex を含むすべての AI コーディングエージェント向けの
リポジトリ運用指示であり、**共通仕様の唯一の正典（Single Source of Truth）** です。
...
- 適用範囲: リポジトリ全体（ルート、`src/`、`_ideas/`、`.github/_roleplay-datas/`、`.agents/`、サブモジュール参照）。
...
**本ファイル（AGENTS.md）が共通仕様の唯一の正典。** プロジェクト概要・編集境界・Git ブランチ運用・実行コマンド・
出力規則・ログ規約・MIME・形態共通データセット・サブモジュール・docs 同期・実務ルール・禁止事項などの
**共通仕様はすべてここに集約**し、ツール別の設定書には**固有事項のみ**を書く。

### エージェント別の読み込み経路

すべてのエージェントが、経路は違っても**最終的に本ファイルへ到達する**ように配線してある。

| エージェント | 自動ロードされる入口 | AGENTS.md への到達経路 | ツール固有の薄い設定書 |
|---|---|---|---|
| **GPT Codex** (CLI / IDE 拡張) | `AGENTS.md`（ネイティブ読込） | **直接**（このファイル自身） | [`CODEX.md`](CODEX.md) ※本ファイルの「前提条件」から読み込みを指示 |
| **GitHub Copilot** (VS Code) | [`.github/copilot-instructions.md`](.github/copilot-instructions.md) ＋ [`.github/instructions/*.instructions.md`](.github/instructions/) (`applyTo: '**'`) | 設定書冒頭のリンク | [`.github/copilot-instructions.md`](.github/copilot-instructions.md) |
| **Claude Code** (CLI / VS Code 拡張) | [`CLAUDE.md`](CLAUDE.md) | 設定書冒頭のリンク | [`CLAUDE.md`](CLAUDE.md) |
| **Claude デスクトップ / Cowork** | [`CLAUDE.Cowork.md`](CLAUDE.Cowork.md)（手動添付 or プロジェクト設定） | 設定書冒頭のリンク | [`CLAUDE.Cowork.md`](CLAUDE.Cowork.md) |

- VS Code で Copilot にも `AGENTS.md` を直接読ませたい場合は、ユーザー設定で `chat.useAgentsMdFile` を有効化する
  （任意。`.vscode/settings.json` は `.gitignore` 対象なのでリポジトリでは強制しない）。
- 新しいエージェントを導入するときは、**この表に 1 行追加**し、対応する薄い設定書を作ってから運用を始めること。

### 薄い設定書のルール

- 薄い設定書には**共通仕様を重複させない**こと。共通ルールを変更するときは必ず本ファイルを更新する。
- 薄い設定書を更新する際に共通事項に触れた場合は、その内容を本ファイルへ巻き取ること。
- 4 つの薄い設定書は互いに「対をなす設定書」として相互リンクを持つ。**1 つ増やしたら 4 か所すべてのリンクを更新**する。
...
| パス | 位置づけ |
|---|---|
| [`.agents/skills/numbertales-imagegen/`](.agents/skills/) | **正本（実体）。** ベンダー中立のスキル配置規約。GPT Codex がプロジェクトスキルとして読む |
| `.claude/skills/numbertales-imagegen/` | **生成ミラー。** Claude Code / Cowork 用。直接編集せず同期スクリプトで反映する |
| `~/.claude/skills/numbertales-imagegen` | パーソナルスキル用ジャンクション（`install-personal-skill.ps1` が作成） |

```powershell
# 差分チェック（CI / 作業前確認。差分ありなら exit 1）
...
- **スキル本文はツール中立に書く。** 「実機 Claude Code」「実機 Codex」のような特定ツール名の断定を避け、
  「実機（Windows / ローカル CLI エージェント）」のように書く。両方に同じファイルが配られるため。
- `repo_path.txt` は環境固有のため両側とも `.gitignore` 済み。同期対象からも除外する。
- 実装: [`scripts/sync-agent-skills.ps1`](scripts/sync-agent-skills.ps1)、詳細は [`docs/agent-config.md`](docs/agent-config.md)。

---

## 前提条件

- **本ファイルを入口として直接読み込むエージェント（GPT Codex など）は、続けて [`CODEX.md`](CODEX.md) も必ず読むこと。**
  ツール固有の事項（承認モード・サンドボックス・スキル配置・課金を伴う実行の扱い）がそこに書かれている。
  他のエージェントはそれぞれの薄い設定書（上表）から本ファイルへ来ているため、追加読込は不要。
- 回答言語は日本語。
...
- `.agents/skills/` はスキルの**正本**。編集はこちらで行い、`.claude/skills/` は**生成ミラーなので直接編集しない**
  （上の「エージェントスキルの配置と同期」参照）。
...
- 各ツールの薄い設定書: [`CODEX.md`](CODEX.md) / [`CLAUDE.md`](CLAUDE.md) / [`.github/copilot-instructions.md`](.github/copilot-instructions.md) / [`CLAUDE.Cowork.md`](CLAUDE.Cowork.md)
- スキル正本: [`.agents/README.md`](.agents/README.md)
...
| **エージェント共通ルールの変更**（口調・境界・禁止事項・運用手順）                       | 本ファイル（AGENTS.md）**のみ**。薄い設定書には書き足さない                                                     |
| **対応エージェントの追加・削除**、薄い設定書の新設                                      | AGENTS.md の読み込み経路表 + 薄い設定書 4 種の相互リンク + [`docs/agent-config.md`](docs/agent-config.md)       |
| **スキル (`.agents/skills/`) の内容変更**                                               | 正本を編集後 `scripts\sync-agent-skills.ps1 -Apply` でミラー同期。`-Check` が通ることを確認                     |
...
5. 生成ミラー `.claude/skills/` の直接編集（正本は `.agents/skills/`。同期スクリプトで上書きされ変更が消える）。
6. 共通仕様を薄い設定書（`CODEX.md` / `CLAUDE.md` / `.github/copilot-instructions.md` / `CLAUDE.Cowork.md`）へ重複記述すること。
