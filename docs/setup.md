...
CANVA_REFRESH_TOKEN=your_canva_refresh_token   # 初回 PKCE 認証後に自動保存。--use-refresh-token で次回以降はブラウザ不要
...
| Canva `401` / `403`                                  | `CANVA_ACCESS_TOKEN` の期限切れ。`CANVA_REFRESH_TOKEN` があれば `python -m src.tools.refresh_canva_token --use-refresh-token` でブラウザ不要で更新。初回または refresh_token もない場合は引数なしで実行（ブラウザ認証）。MCP サーバ側からは `numbertales_refresh_canva_token` ツールで更新可 |
| Linux / GCE で `python: command not found`           | Debian 系は `python3` を使う。または `sudo apt install python-is-python3` でエイリアスを作る |
