...
description: ナンバーテールズ（百花繚乱研究所の一次創作）のキャラクター画像を、このリポジトリのマルチLLM画像生成パイプライン (src.pipeline.image_pipeline) で生成・改稿するためのスキル。ユーザーが「57をコアフォルダで生成して」「図書館シーンの絵を作って」「前回の画像の表情だけ直して」「25と57を並べて」のようにナンバーテールズの作画を依頼したときに使用する。自然文リクエスト・キャラ番号指定・i2i改稿・合同/バッチ生成・出力ログ規則・不変特徴やNCライセンスの遵守を扱う。画像生成プロンプトの組み立てと検証もこのスキルの範囲。
...
3. **Stage 3** ラフ生成（Adobe構図ガイド + Gemini Imagen / 単体5案・合同は3枚×N人）
4. **Stage 4** 違反特徴の分析・除去（OpenAI Vision + Gemini i2i）
5. **Stage 5** 完成画像3枚 + Canva仕上げ

詳細なフラグ・環境変数・出力構成は **REFERENCE.md** を参照すること。

---

## 必ず守る前提（実行前チェック）

1. **作業ディレクトリはリポジトリルート** で実行する（`python -m src.pipeline.image_pipeline ...`）。
...
python -m src.pipeline.image_pipeline \
    --natural "コアフォルダ姿の57(イズナ)が図書館で本を読んでいる絵"
```
...
- 生成画像を扱う前に MIME チェックを推奨:
  ```bash
  python -m src.tools.check_image_mime          # output/ を再帰スキャン
...
- Canva 関連エラー → `--skip-canva` を付けて Stage 5 を回避できる。
