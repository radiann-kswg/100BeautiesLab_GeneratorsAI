...
- VS Code のユーザー設定で `chat.useAgentsMdFile` を有効化すると、Copilot が [AGENTS.md](../AGENTS.md) を直接ロードします（任意・推奨）。`.vscode/settings.json` は `.gitignore` 対象のため、リポジトリからは強制しません。
...
- 対をなす薄い設定書: [CODEX.md](../CODEX.md)（GPT Codex 向け） ／ [CLAUDE.md](../CLAUDE.md)（Claude Code 向け） ／ [CLAUDE.Cowork.md](../CLAUDE.Cowork.md)（Cowork 向け）
